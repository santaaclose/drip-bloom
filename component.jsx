...
const GlassRingsShop = () => {
  ...
  // Скрытие/показ шапки при скролле с плавной анимацией
  useEffect(() => {
    let ticking = false;
    ...
    const handleScroll = () => {
      if (!ticking && !isAnimating) {
        requestAnimationFrame(() => {
          ...
          // Игнорируем мелкие движения для плавности
          if (scrollDelta < 3) {
            ticking = false;
            return;
          }
          
          // Находим высоту шапки
          const headerHeight = 120;
          
          let shouldShow = false;
          
          // Показываем шапку когда:
          // 1. В самом верху (0-40px)
          // 2. Или когда скроллим вверх И находимся выше секции фильтров
          if (currentScrollY <= 40 || (scrollDirection === 'up' && currentScrollY <= headerHeight + 50)) {
            shouldShow = true;
          } 
          // Скрываем шапку когда скроллим вниз И прошли шапку + отступ
          else if (scrollDirection === 'down' && currentScrollY > headerHeight) {
            shouldShow = false;
          } else {
            // Сохраняем текущее состояние если условия не выполнены
          ...
          if (shouldShow !== isHeaderVisible) {
            ...
            // Сброс флага анимации через время анимации
            setTimeout(() => {
              isAnimating = false;
            }, 400);
          }
...
📞 Прошу связаться для обсуждения деталей`;
  ...
  const CatalogPage = () => (
    <div className="min-h-screen" style={{ backgroundColor: '#FAFAFA' }}>
      {/* Header */}
      <header 
        className={`backdrop-blur-sm sticky top-0 z-10 border-b transition-all duration-500 ease-in-out ${
      ...
      >
        <div className="max-w-md mx-auto px-8 py-8">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-medium" style={{ color: '#4A3540' }}>Фильтры</h3>
            <button 
          ...
          {/* Collections */}
          <div className="mb-4">
            <p className="text-xs mb-3 font-medium tracking-wider" style={{ color: '#5A4832' }}>КОЛЛЕКЦИИ</p>
          ...
          {/* Price Range */}
          <div className="mb-4">
            <p className="text-xs mb-3 font-medium tracking-wider" style={{ color: '#5A4832' }}>ЦЕНА</p>
            <div className="flex flex-wrap gap-2">
          ...
          {/* Size */}
          <div className="mb-2">
            <p className="text-xs mb-3 font-medium tracking-wider" style={{ color: '#5A4832' }}>РАЗМЕР</p>
            <div className="flex flex-wrap gap-2">
              {sizes.map(size => (
                <button
                  key={size}
      ...
      <div className="max-w-md mx-auto px-4 py-6">
        <div className="text-sm mb-6 font-medium" style={{ color: '#5A4832' }}>
          ...
          {filteredRings.map(ring => (
            ...
            >
              ...
              <div className="relative">
                ...
                {/* Image Navigation Dots - компактные */}
                ...
                {/* Image Navigation Labels - упрощенные */}
                <div className="absolute top-2 right-2">
                  <div className="bg-black/20 backdrop-blur-sm text-white text-xs px-2 py-1 rounded-full font-medium">
                    {(selectedImageIndex[ring.id] || 0) + 1}/{ring.images.length}
                  </div>
                </div>
                
                {/* Swipe Indicator - только на первой карточке */}
                {ring.id === 1 && (
              ...
              {/* Product Info - компактная версия */}
              <div className="p-4">
                <div className="mb-3">
                  <h3 className="font-medium text-sm mb-1 line-clamp-1" style={{ color: '#3A2A35' }}>{ring.name}</h3>
                  <div className="flex items-center justify-between">
                    <p className="text-xs font-medium" style={{ color: '#5A4832' }}>{ring.collection}</p>
                    <p className="text-sm font-semibold" style={{ color: '#3A2A35' }}>{ring.price}₽</p>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between mb-3 text-xs">
                  <div className="flex items-center gap-1">
                    <span style={{ color: '#4A3B32' }}>📏</span>
                    <span style={{ color: '#4A3B32' }}>{ring.sizes.join(', ')}</span>
                  </div>
                  <div className="flex items-center gap-1">
                    <span style={{ color: '#4A3B32' }}>🎨</span>
                    <span style={{ color: '#4A3B32' }}>{ring.colors.length}</span>
                  </div>
                </div>
